...
import { DanioI, ZoneINTI } from 'src/app/shared/ngx-wizard/workflow/workflow.model';
...
export class IntComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.zoneINTI.headerId = Number(this.id);
    this.zoneINTI.zoneType = this.zoneType;
  ...
  async getZonesDamages() {
    ...
    let response = await this.workflowService.getZonesDamages(this.zoneINTI);

    this.rearType = response.data.rearType;
    this.image = response.data.rearType == 'ABATIBLE' ? 'int' : 'int_cortina';
    ...
    if (!!response.data && !!response.data.zoneData) {
      this.pluginsForm.patchValue({
        hiddenNumber: !!response.data.zoneData.intNumeroOcultos ? response.data.zoneData.intNumeroOcultos : 'N',
        comment: response.data.zoneData.intObservacions
      });
  ...
  async savePlugins(force: boolean = false) {
    ...
    if (this.hasgChangesCircles()) captureImage("INT", this.id, this.fileService);

    if (loadFiles) {
      await this.getFiles();
    }

    this.zoneINTI.numeroOcultos = this.pluginsForm.value.hiddenNumber;
    this.zoneINTI.observacions = this.pluginsForm.value.comment;

    this.zoneINTI.allDanos = this.circles;

    let response = await this.workflowService.patchZonesDamages(this.zoneINTI);
    this.initialState = this.getCurrentStateSnapshot();
  ...
  async addDamage(event: MouseEvent) {
    ...
    const cell = this.damagesModal.getCellFromEvent(event, this.scale);
