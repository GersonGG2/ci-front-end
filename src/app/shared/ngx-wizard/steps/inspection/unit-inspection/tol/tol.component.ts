...
import { DanioI, ZoneTOLI } from 'src/app/shared/ngx-wizard/workflow/workflow.model';
...
@Component({
  ...
  templateUrl: './tol.component.html',
  styleUrls: ['../unit-inspection.component.scss']
})
export class TolComponent implements OnInit {
  id = '';
  itemCode = '';
  ...
  async ngOnInit() {
    ...
    this.zoneTOLI.headerId = Number(this.id);
    this.zoneTOLI.zoneType = this.zoneType;
  ...
  async getZonesDamages() {
    ...
    let response = await this.workflowService.getZonesDamages(this.zoneTOLI);
    ...
    if (!!response.data && !!response.data.zoneData) {
      this.pluginsForm.patchValue({
        hiddenNumber: !!response.data.zoneData.tolNumeroOcultos ? response.data.zoneData.tolNumeroOcultos : 'N',
        comment: response.data.zoneData.tolObservacions
      });
  ...
  async getFiles() {
    ...
    this.initialState = this.getCurrentStateSnapshot(); // Guardar el estado inicial al cargar los datos
  ...
  async savePlugins(force: boolean = false) {
    ...
    if (this.hasgChangesCircles()) captureImage("TOL", this.id, this.fileService);

    if (loadFiles) {
      await this.getFiles();
    }

    this.zoneTOLI.numeroOcultos = this.pluginsForm.value.hiddenNumber;
    this.zoneTOLI.observacions = this.pluginsForm.value.comment;
    this.zoneTOLI.allDanos = this.circles;

    let response = await this.workflowService.patchZonesDamages(this.zoneTOLI);
