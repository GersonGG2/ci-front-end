...
import { DanioI, ZoneCDI } from 'src/app/shared/ngx-wizard/workflow/workflow.model';
...
export class CdComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.zoneCDI.headerId = Number(this.id);
    this.zoneCDI.zoneType = this.zoneType;
  ...
  async getZonesDamages() {
    ...
    let response = await this.workflowService.getZonesDamages(this.zoneCDI);
    ...
    if (!!response.data && !!response.data.zoneData) {
      this.pluginsForm.patchValue({
        hiddenNumber: !!response.data.zoneData.cdNumeroOcultos ? response.data.zoneData.cdNumeroOcultos : 'N',
        comment: response.data.zoneData.cdObservacions
      });
  ...
  async savePlugins(force: boolean = false) {
    ...
    if (this.hasgChangesCircles()) captureImage("CD", this.id, this.fileService);

    if (loadFiles) {
      await this.getFiles();
    }

    this.zoneCDI.numeroOcultos = this.pluginsForm.value.hiddenNumber;
    this.zoneCDI.observacions = this.pluginsForm.value.comment;

    this.zoneCDI.allDanos = this.circles;

    let response = await this.workflowService.patchZonesDamages(this.zoneCDI);
    this.initialState = this.getCurrentStateSnapshot();
