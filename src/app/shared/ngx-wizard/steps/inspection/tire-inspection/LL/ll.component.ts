...
import { captureImage } from '../../unit-inspection/unit-inspection.component';

@Component({
  selector: 'app-ll',
  imports: [CommonModule, ReactiveFormsModule, FormsModule, NgbNavModule, DamagesTiresModalComponent, NgSelectComponent, ImgRetryComponent],
  templateUrl: './ll.component.html',
  styleUrls: ['../tire-inspection.component.scss']
})
export class LLComponent implements OnInit {

  //parametro de entrada tipo de llanta
  @Input() zoneType: string = '';
  ...
  async ngOnInit() {
    ...
    let inspectionResponse = await this.workflowService.getIncomingInspection(this.id);
    ...
    this.number = this.zoneType.replace('LL', '');
  ...
  async savePlugins(force: boolean = false): Promise<boolean> {
    ...
    if (this.hasgChangesCircles()) captureImage(`LL${this.number}`, this.id, this.fileService);
  ...
  async addPhotos(event: any, index: number | string) {
    ...
    await this.fileService.uploadFiles(this.module, Number(this.id), `${this.module}/${this.zoneType}/${index}/${Number(this.id)}/`, files);
    await this.getFiles();
  ...
  async findByEcoNumber() {
    this.pluginsForm.get('eco')?.markAsTouched();
    ...
    try {
      ...
      if (!!response.data) {
        this.pluginsForm.patchValue({
          type: response.data.tipo || null,
          eco: response.data.eco || null,
          dot: response.data.dot || null,
          brand: response.data.marca || null,
          //brand: response.data.insMarca,
          model: response.data.modelo || null,
          measurement: response.data.medidas || null,
          renewed: response.data.renovados,
          min: response.data.val_1_32_1 || null,
          max: response.data.val_1_32_2 || null,
          condition: response.data.condicion,
          comment: response.data.comments || null
        });
      } else {
        this.toastr.error('Datos no encontrados', 'Aviso');
      }
    } catch (error) {
      this.toastr.error('Datos no encontrados', 'Aviso');
    }
  ...
  private getCurrentStateSnapshot() {
    return JSON.stringify({
      ...
      complem: this.pluginsForm.value,
    });
  }
